 ...
 * LastEditTime: 2019-11-03 10:54:38
...
bool isGoodZBoson(const vector<Particle> &mZ_daughters, Particle &ZBoson,
                  const double &ZBosonPtMin = 60.0) {
    if (mZ_daughters.size() != 2) {
        ZBoson.reset(0., 0., 0., 0.);
        return false;
    } else if (abs(mZ_daughters[0].pdg_id()) == 11 &&
               abs(mZ_daughters[1].pdg_id()) == 11) {
        //
        if ((fabs(mZ_daughters[0].eta()) < 1.44 ||
            ...
            (fabs(mZ_daughters[1].eta()) < 1.44 ||
             (fabs(mZ_daughters[1].eta()) > 1.57 &&
              fabs(mZ_daughters[1].eta()) < 2.5)  // daughter1
             )) {
            ZBoson = mZ_daughters[0] + mZ_daughters[1];
            if (mZ_daughters[0].pt() > 20.0 && mZ_daughters[1].pt() > 20.0 &&
                ZBoson.m() > 70.0 && ZBoson.m() < 110 &&
                ZBoson.pt() > ZBosonPtMin && fabs(ZBoson.rap()) < 2.5) {
                return true;
            } else {
                ZBoson.reset(0., 0., 0., 0.);
                return false;
    ...
    } else if (abs(mZ_daughters[0].pdg_id()) == 13 &&
               abs(mZ_daughters[1].pdg_id()) == 13) {
        //
        if (fabs(mZ_daughters[0].eta()) < 2.4 &&
            fabs(mZ_daughters[1].eta()) < 2.4 && mZ_daughters[0].pt() > 10.0 &&
            mZ_daughters[1].pt() > 10.0) {
            //
            ZBoson = mZ_daughters[0] + mZ_daughters[1];
...
bool isGoodZBoson(const vector<Particle> &mZ_daughters, Particle &ZBoson,
                  const double &ZBosonPtMin, const double &ZBosonPtMax) {
    Particle tmpZBoson;
