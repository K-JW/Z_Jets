 ...
 * LastEditTime: 2019-11-03 11:03:54
...
int main(int argc, char* argv[]) {
    // 参数解析
    cmdline::parser args;
    args.add<string>("out-dir", 'o', "assigned the path of output data", false,
                     "./Output");
    args.add<double>("Z-pt-Min", 's',
                     "assigned select Z's mix scalar transverse momentum",
                     false, 60.0);
    args.add<double>("Z-pt-Max", 'g',
                     "assigned select Z's max scalar transverse momentum",
                     false, inf);
    ...
    if (argc == 1 || args.exist("help")) {
        cerr << args.usage();
        cout << "\033[1;31mERROR:\033[0m Please supply HepMC files as "
                "argument...\n";
        exit(EXIT_FAILURE);
    }
    if (!isGetArgs) {
    ...
    const double ZptMin = args.get<double>("Z-pt-Min");
    const bool hasZptMax = args.exist("Z-pt-Max");
    const double ZptMax = args.get<double>("Z-pt-Max");
    ...
    if (file_num < size) {
        cerr << "--\033[1;31m ERROR:\033[0m Assigned process number greater "
                "than files number, please check it!"
             << endl;
        exit(EXIT_FAILURE);
    }
    ...
    MPI_Scatterv(global_file_index, send_count, offset_array, MPI_INT,
                 locale_file_index, send_count[rank], MPI_INT, 0,
                 MPI_COMM_WORLD);
    ...
    for (int i = 0; i < send_count[rank]; i++) {
        ...
        std::cout << "Rank: " << rank << ", File name: " << hepmc_in
                  << std::endl;

        // 输出文件名
        std::string output =
            args.get<string>("out-dir") + "/" +
            iHepTools::get_file_name(static_cast<std::string>(hepmc_in)) +
            static_cast<std::string>("Rank-") + std::to_string(rank) +
        ...
        while (evt) {
            ...
            if (mZ_daughter.size() == 2) {
                iHepTools::Particle ZPoolTemp;
                const bool goodEvent =
                    !hasZptMax ? iHepTools::isGoodZBoson(mZ_daughter, ZPoolTemp,
                                                         ZptMin)
                               : iHepTools::isGoodZBoson(mZ_daughter, ZPoolTemp,
                                                         ZptMin, ZptMax);
                if (goodEvent) {
                    fact_event_number++;
                    isGoodEvent = true;
            ...
            if (isGoodEvent) {
                iHepTools::WriteToHepMC2(
                    out, fact_event_number, evt->alphaQCD(), evt->alphaQED(),
                    evt->weights()[0], evt->weights()[1], evt->weights()[2],
                    evt->weights()[3], evt->weights()[4], mZ_daughter,
                    mParticles);
            }
...
string iHepTools::get_file_name(string filepath) {
    if (!filepath.empty()) {
        ...
        std::string file_name = filepath.substr(
            location_filename + 1, location_point - location_filename - 1);
        return file_name;
    } else {
        return "NULL";
    }
